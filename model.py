...
class MltcNet(nn.Module):

    def __init__(self, args):
        super(MltcNet, self).__init__()
        self.bert = get_bert(args)
        self.feature_layers=args.feature_layers
        self.clf = nn.Linear(self.feature_layers * self.bert.config.hidden_size, args.label_size)
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, labels=None):
        ...
        logits = torch.sigmoid(self.clf(features))
...
class Extractor(nn.Module):
    ...
    def forward(self, inputs):
        ...
        feature = torch.cat(inputs, 0)				
        return feature, torch.cat(batch_scores, 0)
...
class Y_S(nn.Module):
    ...
    def forward(self, bags, musk_idxs, pos1, pos2):
        ...
        feature, scores = self.extractor(groups)	
        if self.sigmoid:			
...
class RE(nn.Module):
    ...
    def baseModel(self, bags, musk_idxs, pos1, pos2, labels):
        features, pred = self.y_s(bags, musk_idxs, pos1, pos2)
